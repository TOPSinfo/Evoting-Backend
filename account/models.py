...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    status = models.IntegerField(
        _("User Status"), default=USER_STATUS[0][0], choices=USER_STATUS
    )
...
class UserProfile(models.Model):
    ...
    voter_id = models.CharField(
        max_length=255, verbose_name="voter_id"
    )
    aadhar_no = models.CharField(
        max_length=255, verbose_name="aadhar_no"
    )
    age = models.IntegerField(_("User Age"), default=0)
    ...
    @staticmethod
    def get_profile_by_address(add=None):
        try:
            return UserProfile.objects.get(address=add)
        except:
            return None
